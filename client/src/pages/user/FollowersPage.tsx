        ...
        {followers.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-[#6b7280] dark:text-[#9ca3af] max-w-sm">
              @{username} doesn't have any followers yet. Be the first to follow!
            </p>
          </div>
        ) : (
          /* ─── Followers Grid ───────────────────────────────── */
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {followers.map((user: User) => (
              <Link
                key={user.id}
                to={`/u/${user.username}`}
                className="group flex flex-col items-center p-6 rounded-2xl bg-[#f9fafb] dark:bg-[#111111] hover:bg-[#f3f4f6] dark:hover:bg-[#1a1a1a] transition-all duration-200 border border-transparent hover:border-[#e5e7eb] dark:border-[#2a2a2a]"
              >
                ...
                <div className="mt-4 text-center min-w-0">
                  ...
                  {user._count && user._count.followers !== undefined && (
